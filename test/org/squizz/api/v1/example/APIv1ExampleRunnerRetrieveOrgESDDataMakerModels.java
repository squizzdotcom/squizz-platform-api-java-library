...
/**
 * Shows an example of creating a organisation session with the SQUIZZ.com platform's API, then retrieves Maker Model data from a connected organisation in the platform
 */
public class APIv1ExampleRunnerRetrieveOrgESDDataMakerModels
{
    public static void main(String[] args)
    {
		...
		String result = "FAIL";
		HashMap<String, ESDRecordMaker> makersRecordIndex = new HashMap<>();
		...
		{
			System.out.println("Attempting to obtain maker data.");
			...
			while(getMoreRecords)
			{
				...
				System.out.println("Attempt made to obtain maker data. Page number: "+pageNumber);
				...
				{	
					...
					if(esDocumentMaker.dataRecords != null)
					{
						...
						for (ESDRecordMaker makerRecord: esDocumentMaker.dataRecords){
							makersRecordIndex.putIfAbsent(makerRecord.keyMakerID, makerRecord);
						}
		...
		if(result.equals("SUCCESS"))
		{
			...
			while(getMoreRecords)
			{
				...
				//check that the data successfully retrieved
				if(endpointResponseESD.result.equals(APIv1EndpointResponse.ENDPOINT_RESULT_SUCCESS)){
					System.out.println("SUCCESS - maker model data successfully obtained from the platform for page number: "+pageNumber);
					...
					System.out.println("Maker Model Records Returned: " + esDocumentMakerModel.totalDataRecords);

					//check that records have been placed into the standards document
					if(esDocumentMakerModel.dataRecords != null){
						System.out.println("Maker Model Records:");
						...
						{    
							...
							if(makersRecordIndex.containsKey(makerModelRecord.keyMakerID)){
								makerName = makersRecordIndex.get(makerModelRecord.keyMakerID).name;
							}
							
							//output details of the maker model record
							System.out.println(APIv1OrgTestRunner.CONSOLE_LINE);
							System.out.println("    Maker Model Record #: " + recordNumber);
							System.out.println("      Key Maker Model ID: " + makerModelRecord.keyMakerModelID);
							System.out.println("              Model Code: " + makerModelRecord.modelCode);
							System.out.println("          Model Sub Code: " + makerModelRecord.modelSubCode);
							System.out.println("              Model Name: " + makerModelRecord.name);
							System.out.println("            Key Maker ID: " + makerModelRecord.keyMakerID);
							System.out.println("              Maker Name: " + makerName);
							System.out.println("       Model Search Code: " + makerModelRecord.modelSearchCode);
							calendar.setTimeInMillis(makerModelRecord.releasedDate);
							System.out.println("           Released Date: " + new SimpleDateFormat("dd/MM/yyyy").format(calendar.getTime()));
							calendar.setTimeInMillis(makerModelRecord.createdDate);
							System.out.println("            Created Date: " + new SimpleDateFormat("dd/MM/yyyy").format(calendar.getTime()));
							
							//check if the model contains any attributes
							if(makerModelRecord.attributes.size() > 0)
							{
								//output attributes
								System.out.println("       Attributes Values: " + makerModelRecord.attributes.size());

								//output each attribute value
								int attributValueCount = 0;
								for (ESDRecordAttributeValue attributeValueRecord: makerModelRecord.attributes)
								{						
									attributValueCount++;
